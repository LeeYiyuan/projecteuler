...
def check_jobs(value):
    ivalue = int(value)
    if ivalue <= 0:
         raise argparse.ArgumentTypeError('Invalid argument for JOBS: {0}.'.format(value))
    return ivalue
...
def check_solution(value):
    ...
    ivalue = int(value)
    if ivalue <= 0:
         raise argparse.ArgumentTypeError('Invalid argument for SOLUTION: {0}.'.format(value))
    return value
...
solutions_stack = queue.LifoQueue()
solutions_count = 0
results = []

for solution in [f.strip('.cpp') for f in sorted(os.listdir(), reverse=True) if re.match('[0-9]+.cpp', f)] if 'ALL' in args.solutions else args.solutions:
    solutions_stack.put(solution)
    solutions_count += 1

def worker():
    while True:
        solution = solutions_stack.get()
        if solution is None:
            break
