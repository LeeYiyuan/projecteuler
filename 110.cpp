/*
    It is possible to characterize every positive integer according to their unique
    prime factorizations.

    Let the unique prime factorization of some positive integer n be
    [e_2, e_3, e_5, ...] where n = 2^e_2 * 3^e_3 * 5^e_5 *.... Then the number
    of unique factor pairs of n^2, fp(n^2) disregarding order, is
    ...
    is reached if either of the conditions are met:
        ...
        1) e_i >= e_{i - 1}. This is because if, for example e_5 > e_3, we can
           swap e_5 with e_3 to get a smaller n while maintaining fp(n^2). As such
           we are only concerned about non-increasing (e_2, e_3, e_5, ...).
        2) The computed value of fp(n^2) using the exponents thusfar exceeds 4000000.
           This is because appending any more exponents will only cause n to
           increase while fp(n^2) remains larger than 4000000; And thus, any further
           n due to appended exponents will not be smaller than the current hit
           value of n.
        3) The computed value of n using exponents thusfar exceeds the n value of
           hits previously found. This is because adding any more exponent will
           cause the current value of n to be more than that of a previously
           found hit, so it will not be the smallest value of n.

    We take the minimum n among leaves such that e_i <= e_{i - 1} and fp(n^2) > 4000000.
*/
